...
| **B17** | Live smoke | **RUN 2026-08-08 — NOT CLEARED** | Executed against the deployed function after merge `6d9c676` (deployment `dpl_AUzxC1uMaJQ6JG8ijTkFP4n9iTk7`, READY): **15 of 15** real audits through `crawlmouse.com`, all completed, verified in the production DB. **PASS:** pipeline completes on the deployed worker, JS shells refuse on `no_observed_links`, small sites refuse with the correct small-vs-truncated split, healthy sites still grade (hafiz.dev A−/87.59 over 389pg; freepltn C−/56.66 over 219pg, which was predicted to refuse and did not — the prediction was stale, not the engine), `refusal`+`coverage` persist, frontier tables stay 0/0, no new Sentry issues. **FAIL:** (1) a healthy 214-page site with 3,978 links (quotes.toscrape.com) is REFUSED as `site_too_small_to_measure` because kind-based exclusion (pagination 152 / archive 60) collapses `gradeable` to 1 — a false cause on the primary screen, on the WordPress-shaped core market; (2) `fingerprint` is never persisted (0 of 231 audits) because `analyzeCrawl` drops `crawlOut.fingerprint`. Full record: `evidence/2026-08-08-post-merge-b17-live-sample.md`. |
