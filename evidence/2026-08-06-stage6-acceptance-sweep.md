...
| **B10** | Sitemap orphans | **MET — after a gate-4 correction; read the note** | `sitemap-delta-acceptance.test.ts` — delta leads the findings, survives a refusal, robots-disallowed counted **separately** (an owner choice). **This row previously read MET on a vacuous budget-independence test and was wrong to.** Gate 4 / B-A: `sitemapUnreached` was differenced against `GraphAnalysis.depths`, and `buildGraph` drops edges to unfetched targets, so the count was a function of our page cap — measured 36 of 41 unreached at cap 5, 31 at cap 10, 0 at cap 41, on a fixture where *every page links to every other page*. The test claiming to exclude this passed only because its own fixture's leaves had no inbound link **at any cap**, so the cap could never bind. Fixed: reachability is now measured one hop past the fetch boundary (`linkReachableUrls`), and the property is carried by a **new interlinked fixture where the cap does bind** — 0 unreached at caps 5, 10 and 41. The freepltn positive (820 of 821) is unchanged. Three mutations verified: reverting the call site (2 red), growing the reachable set in place (2 red — this is also the §6.6 arrival-order violation), ignoring links entirely (6 red). |
...
| **B12** | Refusal | **MET** | `refusal.test.ts` (the gate), `refusal-surfaces.test.ts` (25 tests covering surfaces 3-8, 10-12, byte-level at the serialization boundary; surfaces 1, 2 and 13 are pinned in `mint-snapshot.test.ts`, `og-report-model.test.ts` and `dashboard.test.ts` respectively — surfaces 1, 2 and 13 are covered elsewhere as listed; **surface 9 — the result page — had NO refusal test at all until this stage**, which is where two live defects were found), and new this stage: `refusal-gate-import-graph-guard.test.ts`, which derives the surface set from the import graph so a fourteenth surface is covered on the day it is imported. |
