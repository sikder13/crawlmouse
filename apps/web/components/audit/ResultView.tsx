...
/**
 * `view === 'ai'` moves the AI-readiness section to the hero position for visitors who arrived from
 * /ai-readiness-checker. Everything else keeps its relative order, and the sections are built ONCE
 * into an array that the two orders index differently — duplicating the JSX per view is how the two
 * would quietly fork the first time one of them was edited.
 *
 * Absent (every homepage audit, and every existing link) the array is today's order exactly.
 */
export function ResultView({ audit, view }: { audit: ClientAuditV2; view?: 'ai' }) {
  ...
  // Built once. `null` entries drop out below exactly as `{cond && <X/>}` did.
  const grade = (
    <GradeReveal
      grade={audit.grade}
      score={audit.score}
  ...
  const graph = <LinkGraphSlot graph={audit.graph} />;
  ...
  const ai = audit.aiReadiness ? (
    <AiReadinessSection aiReadiness={audit.aiReadiness} auditId={audit.id} />
  ) : null;

  const share = <ShareSurface grade={audit.grade} score={audit.score} auditId={audit.id} />;
  ...
  const stay = !audit.viewerSignedIn ? <SaveAndMonitorCta /> : null;
  ...
  // An AI-first arc needs something to lead with. An audit from the v1 engine has no aiReadiness, so
  // the hint falls back to the standard order rather than opening on an empty hero.
  const aiFirst = view === 'ai' && ai !== null;

  // Built unconditionally, and appearing in the AI arc's array only. Its placement is the SINGLE
  // decision about whether it shows: guarding both the construction and the placement made either
  // guard removable without changing behaviour, which is a mutation no test can catch.
  const bridge = (
    <p className="text-body text-ink-muted">
      Your AI-Readiness score measures whether AI can read this site. The internal-linking grade below
      measures whether crawlers can reach it &mdash; the two work together.
    </p>
  );

  const blocks: { key: string; node: ReactNode }[] = aiFirst
    ? [
        { key: 'ai', node: ai },
        { key: 'ai-bridge', node: bridge },
        { key: 'grade', node: grade },
        { key: 'body', node: body },
        { key: 'graph', node: graph },
        { key: 'share', node: share },
        { key: 'stay', node: stay },
        { key: 'notes', node: notes },
      ]
    : [
        { key: 'grade', node: grade },
        { key: 'body', node: body },
  ...
  return (
    <div className="space-y-8">
      {blocks.map(({ key, node }) => (node === null ? null : <Fragment key={key}>{node}</Fragment>))}
    </div>
  );
}
