...
const INVENTORY: [entry: string, why: string][] = [
  ...
  ["inngest/progress.ts :: return raw.slice(0, cut) + '\u2026';",
   "crawled text, cut WITHOUT the shared helper \u2014 PRE-EXISTING, tracked as FU-7"],
  ["apps/web/components/ai/ai-view-logic.ts :: if (chars.length > 96) head = `${chars.slice(0, 95).join('').trimEnd()}\u2026`;",
   "ARRAY slice over Array.from() code points \u2014 cannot split a surrogate pair; RENDER-only, never persisted"],
  ["apps/web/components/ai/ai-view-logic.ts :: let head = (cut > 0 ? text.slice(0, cut) : text).replace(/[.\\s]+$/, '');",
   "cut at a regex-located sentence/em-dash boundary, not a fixed index \u2014 lands between code points by construction; RENDER-only, never persisted"],
