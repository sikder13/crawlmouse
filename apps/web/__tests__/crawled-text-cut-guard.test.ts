...
const INVENTORY: [entry: string, why: string][] = [
  ...
  ["packages/engine/src/analysis/ai-readiness/llms-txt.ts :: const scan = body.length > LLMS_TXT_SCAN_CAP ? body.slice(0, LLMS_TXT_SCAN_CAP) : body;",
   "scan buffer only \u2014 matched by a regex, never persisted"],
  ["packages/engine/src/analysis/ai-readiness/main-content.ts :: else if (sel.startsWith('#')) ids.add(sel.slice(1));",
   "OUR OWN selector constant \u2014 a compile-time ASCII literal, never crawled text"],
  ["packages/engine/src/analysis/ai-readiness/main-content.ts :: else if (sel.startsWith('.')) classes.add(sel.slice(1));",
   "OUR OWN selector constant \u2014 a compile-time ASCII literal, never crawled text"],
